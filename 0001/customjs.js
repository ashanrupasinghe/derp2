	   ...
	   /* $( ".btnRemove" ).click(function() {
	    	...
	    	if(quantity && productId){
		    	 $.post(myBaseUrl+"orders/singlecal",
		 	    	    ...
		 	    	    function(data, status){
		 	    	    	...
		 	    	    	var singleTotalCostString=productTotalPrice['productCost']+" X "+productTotalPrice['productQuantity']+" = "+productTotalPrice['productTotalCost'];
		 	    	    	$("#"+divid).find("input.product-ammount").val(totalstring);
		 	    	    	$("#"+divid).find("input.product-ammount-hidden").val(productTotalPrice['total']);
		 	    	    	$("#"+divid).find("span.single-cost").text("(Cost: "+singleTotalCostString+")");
		 	    	    	$("#"+divid).find("input.product-cost-hidden").val(productTotalPrice['productTotalCost']);
	    ...
	    function displaySubTotal(){
	    	...
	    	// alert(discount);
	    	 ...
	    	 //totalcost
	    	 var productCost = $('input[name="product_cost[]"]');
	    	 var totalCost=0;
	    	 for(var i = 0; i < productCost.length; i++){
	    		 totalCost+=parseInt(($(productCost[i]).val()));
	    		}
...
/*		            	    	listItems= "<option value=''>select supplier</option>";*/
		      		          ...
		      		          var row='<div class="group col-lg-12 col-md-12 columns prod" id="x-'+element.value+'" style="padding-left:0px; padding-right:0px;">'+
			          			...
			          			'<div class="col-lg-3 col-md-3 columns form-group">'+
			          				...
			          				'<span class="small-cost single-cost"></span>'+
			          				'<input name="product_cost[]" disabled="disabled" class="form-control product-cost-hidden" id="" value="0" type="hidden">'+
			          			'</div>'+
...
/**
 * when removing item form multy slect dropdown, counting total again::just for display
 */
$("select#product-list").on("select2:unselecting", function (e) {
	displaySubTotalWithTimeout();
	$(this).select2('close');
});
