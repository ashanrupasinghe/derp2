<style>
/* select 2 hidden dropdown */
.x-y-z .prod-left .select2-container {
                  ...
                  <div class="x_content">
                   ...
                   <div class="group col-lg-12 col-md-12 columns prod x-y-z" id="x-<?= $product->product_id ?>" style="padding-left:0px; padding-right:0px;">
			          			<div class="col-lg-3 col-md-3 columns prod-left form-group">
			          				...
			          				<?php /*?>
			          				<input type="hidden" class="form-control" name="product_name[]" value="<?= $product->product_id ?>">
			          				<?php  */
			          				?>
			          				<select name="product_name[]" class="form-control select2-display-none select-2-order-edit-product orders-products-id">
			          			...
			          			<input name="product_price[]" disabled="disabled" class="form-control product-ammount-hidden" id="" type="hidden" value="<?= $product->product_price*$product->product_quantity ?>">
			          			<span class="small-cost single-cost">(Cost: <?= $product_cost[$product->product_id].' X '.$product->product_quantity.' = '.$product_cost[$product->product_id]*$product->product_quantity ?>)</span>
			          			<input name="product_cost[]" disabled="disabled" class="form-control product-cost-hidden" id="" value="<?= $product_cost[$product->product_id]*$product->product_quantity ?>" type="hidden">
			          			<?php $totalcost+=$product_cost[$product->product_id]*$product->product_quantity;?>
			          			<!-- product codt lines need to change in edit part,
			          			now we get the cost from current product details,
			          			but there should be product cost @ ordered time -->
			          			</div>
...
<div class="col-md-8 col-sm-8 col-xs-12">
	<?php echo $this->Form->input('discount',['default'=>$order->discount,'label' => false,'class'=>'form-control','disabled'=>true, 'type'=>'hidden']); ?>
	<?php echo $this->Form->input('direct_discount',['default'=>$order->discount,'label' =>false,'class'=>'form-control','disabled'=>false]); ?>
</div>
</div>
<div class="form-group">
...
<div class="col-md-8 col-sm-8 col-xs-12">
	...
	<span class="small-cost total-cost">(Total Cost: <?= $totalcost ?>)</span>
</div>
</div>	
</div>    
...
?>
<div class="form-group">
	<label class="control-label col-md-4 col-sm-4 col-xs-12">Payment Method</label>
	<div class="col-md-8 col-sm-8 col-xs-12">
	<?php
	$pm=[1=>'Cash On Delivery',2=>'Card',3=>'Online'];
	?>
	<?php echo $this->Form->input('payment_method',['label' => false,'class'=>'form-control','options'=>$pm,'empty'=>'select method']); ?>
	</div>	
</div>
