...
class CartController extends AppController {
    ...
    public function beforeFilter(\Cake\Event\Event $event) {
        ...
        $this->Auth->allow([
        	...
        	'guestCheckout',
        	'guestAddAddress',
        	'guestUpdateDeliveryTime',
        	'guestCompleteCheckout',
        	'guestUpdateAddress',
        		'testgenie'					
        ]);
    }
...
//                    $cart_product->qty = $current_product_qty+$product_qty;
    ...
    public function getAddress($isguest=false) {
    	if (!$isguest){
        $token = $this->__getToken();
        $chck = $this->__checkToken($token);
    	}
        if ((!$isguest && $chck ['boolean']) || $isguest) {
			if (!$isguest){
            $cart_id = $this->__getCurrentCartId($chck ['user_id']);
			}else{
				$session = $this->request->session();
    ...
    public function guestUpdateAddress() {
    	$this->request->allowMethod([
    			'post'
    	]);
    	header('Content-type: application/json');
    			...
    			*/
    			$currrent_shipping_details = $shippingModel->find('all', [
    					...
    					if (sizeof($currrent_shipping_details) > 0) {
    						...
    						$currrent_shipping->email = $address->email;
    						$currrent_shipping->guest_name = $address->guest_name;
    					} else {
    						$data = [
    								'cart_id' => $cart_id,
    								'street_number' => $address->street_number,
    ...
    public function guestAddAddress() {
    	$this->request->allowMethod([
    			'post'
    	]);
    	header('Content-type: application/json');
    		...
    		if ($cart_id) {
    			...
    			$currrent_shipping_details = $shippingModel->find('all', [
    					...
    					if (sizeof($currrent_shipping_details) > 0) {
    						...
    						$currrent_shipping->country = $data ['country'];
    						$currrent_shipping->phone_number = $data ['phone_number'];
    						$currrent_shipping->email = $data ['email'];
    						$currrent_shipping->guest_name = $data ['guest_name'];
    ...
    public function guestUpdateDeliveryTime() {
    	$this->request->allowMethod([
    			'post'
    	]);
    	header('Content-type: application/json');
        ...
        if ($chck ['boolean']) {
            ...
            if ($cart_id) {
                if ($this->__iscartEmpty($cart_id)) {
                    ...
                    // echo 'ssss';
                    $order_id = $this->__addOrder($cart_id, $chck ['user_id'],$payment_method);
                    if ($order_id) {
    ...
    public function guestCompleteCheckout() {
    	$this->request->allowMethod([
    			'post'
    	]);
    	header('Content-type: application/json');
    	$payment_method=$this->request->data('payment_method');
    		...
    		if ($cart_id) {
    			if ($this->__iscartEmpty($cart_id)) {
    				...
    				// add shipping details
    				// echo 'ssss';
    				$order_id = $this->__addOrder($cart_id, null,$payment_method,true);
    				if ($order_id) {
    					$addOrderProducts = $this->__addOrderProducts($cart_id, $order_id);
    					if ($addOrderProducts) {
    						// update shipping order id
    						$this->__updateShippingOrderId($cart_id, $order_id);
    						if ($this->__clearCart($cart_id)) {
    							$this->__sendNotification($order_id, $cart_id, null,true);
    							$return ['status'] = 0;
    							$return ['message'] = "success";
    						} else {
    							$return ['status'] = 906;
    ...
    function __addOrder($cart_id, $user_id=null,$payment_method=1,$isguest=false) {
        $shippingModel = $this->loadModel('Shipping');
        $currrent_shipping_details = $shippingModel->find('all', [
                    'conditions' => [
                        'cart_id' => $cart_id,
        ...
        $order = [
            'customerId' => $isguest ? null:$this->getCustomerID($user_id),
        ...
        if ($orderSaved) {
				...
				/*if ($payment_method==3){
        		$this->Flash->success ( __ ( 'Thank you for your order, we will mail you a payment link once the order is processed' ) );
				}else {
					$this->Flash->success ( __ ( 'Thank you for your order, we will processed the order' ) );
				}*/
    ...
    public function getCheckout($isGuest=false) {
    	if (!$isGuest){
        $token = $this->__getToken();
        $chck = $this->__checkToken($token);
    	}
    ...
    public function guestCheckout() {  
    	$address_book = $this->getAddress(true);
    	$session = $this->request->session();
    	$cart_id = $session->read('cart_id');
    	$total = $this->__getTotal($cart_id);    	
    	$address_book = $address_book ['result'];
    	...
    	$get_checkout = $this->getCheckout(true);
                ...
                if (sizeof($order) > 0) {
                    ...
                    if (sizeof($cart_products) > 0) {
                        ...
                        if ($cartProductModel->saveMany($cartPrdoductsEntities)) {
                            ...
                            $return ['status'] = 0;
                            $return ['message'] = 'go to checkout process to complete reorder';
                            $return ['result'] ['product_list'] = $cart_products;
                            $return ['result'] ['cart_size'] = sizeof($cart_products);
                            $return ['result'] ['total'] = $this->__getTotal($cart_id);
                        } else {
    ...
    private function __sendNotification($orderId, $cart_id, $user_id=null,$isguest=false) {
        ...
        $this->sendToAll2($orderId, 'new', $supplerids, $order->deliveryId, "", $cart_id, $user_id,true); // send emails,product_name,product_quantity,product_supplier
    }
     ...
     */
    public function sendToAll2($orderId, $trype, $suppliers, $delivery, $orderdata = null, $cart_id = null, $user_id = null,$isguest=false) {
        ...
        $customer_mail [$user->username] = $order_data;
			...
			$shippingModel = $this->loadModel('Shipping');
			$shiping = $shippingModel->find('all', ['conditions'=>['order_id'=>$id_of_the_order]])->toArray();
			if (sizeof($shiping)>0){
			$customer_mail [$shiping[0]['email']] = $order_data;
			}else {
				$customer_mail=[];
			}
		}
        ...
        $this->sendemail2('new', $admin_mail, 'admin'); // delivery email
   		 ...
   		 if((!$isguest && filter_var($user->username, FILTER_VALIDATE_EMAIL)) || ($isguest && sizeof($customer_mail)>0)){//if username is a valid email then send email
     	   $this->sendemail2('new', $customer_mail, 'cus'); // delivery email, system has addedd some customers including phone number instead of email for user name, so we have to check that
        }
        // die();
    }
    ...
    public function testgenie(){
    	...
    	print  '<pre>';
    	print_r($order->shipping[0]->guest_name);
    	die();
    }
    
/*function for guest user*/
    //add address->address, email, phone, f name, l name
    //time slot
    //payment method
    //compleate   
