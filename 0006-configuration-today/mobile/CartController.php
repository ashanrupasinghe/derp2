...
class CartController extends AppController {
    ...
    function __getUnavailableDates() {
		...
		//check is today enable for process an order
		 $config_model=$this->loadModel('BoolianConfigurations');
		 $enacle_today=$config_model->find('all',['conditions'=>['config_key'=>'enable_today']])->first();
		 if($enacle_today->config_value==0){
			$dates[]['date']=date("Y-m-d\TH:i:s", time());
		 }
